...
# LAST WORKED ON 3/9/2020 FOR CMIN T3 BY AP
...
cminT3 <- read_csv("raw-data/nsfms_Cmin_t3.csv")
cminT3$irga.integral <- as.numeric(cminT3$irga.integral)
...
cminT3[is.na(cminT3$std.value.1)==FALSE,] -> stds
...
stds <- stds %>% select(labID, meanStandard)


# Assign standard values at all places in between
right_join(stds,cminT3) %>% fill(meanStandard, .direction = "up") -> cminT3


# 1. calculate co2 flux for all microcosms
cminT3 %>%
  mutate(
    time.sampled = paste(date.sampled, time.sampled, sep = " "),
    time.flushed = paste(date.flushed, time.flushed, sep = " "),
  ...
  ) %>% select(c(labID, plotID, soil.volume, CO2CperHour)) -> cminT3_calc 

write.csv(cminT3_calc, "cminT3_raw_calc.csv")

# 2. calculate co2 flux for standards
left_join(microcosmDesign, cminT3_calc, by = "labID") %>% 
...
left_join(microcosmDesign, cminT3_calc, by = "labID") %>% 
  ...
  select(plotID, microcosmNum, Rep, labID, moistTRT, CO2CpergLitter) -> cminT3_corrected_calc  
write.csv(cminT3_corrected_calc, "nsfms_Cmin_t3_calc.csv") 
...
aggregate(. ~ plotID + moistTRT,
          data =
            cminT3_corrected_calc %>%
            select(-c("microcosmNum", "Rep", "labID")),
          FUN = mean
) -> cminT3_aggregate_calc
write.csv(cminT3_aggregate_calc, "nsfms_Cmin_t3_aggregate_calc.csv")
