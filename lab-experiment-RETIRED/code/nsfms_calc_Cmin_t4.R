...
# LAST WORKED ON 3/9/2020 FOR CMIN T4 BY AP
...
cminT4 <- read_csv("raw-data/nsfms_Cmin_t4.csv")
cminT4$irga.integral <- as.numeric(cminT4$irga.integral)
...
cminT4[is.na(cminT4$std.value.1)==FALSE,] -> stds
...
right_join(stds,cminT4) %>% fill(meanStandard, .direction = "up") -> cminT4
...
cminT4 %>%
  ...
  ) %>% select(c(labID, plotID, soil.volume, CO2CperHour)) -> cminT4_calc 

write.csv(cminT4_calc, "cminT4_raw_calc.csv")

# 2. calculate co2 flux for standards
left_join(microcosmDesign, cminT4_calc, by = "labID") %>% 
...
left_join(microcosmDesign, cminT4_calc, by = "labID") %>% 
  ...
  select(plotID, microcosmNum, Rep, labID, moistTRT, CO2CpergLitter) -> cminT4_corrected_calc  
write.csv(cminT4_corrected_calc, "nsfms_Cmin_t4_calc.csv") 
...
aggregate(. ~ plotID + moistTRT,
          data =
            cminT4_corrected_calc %>%
            select(-c("microcosmNum", "Rep", "labID")),
          FUN = mean
) -> cminT4_aggregate_calc
write.csv(cminT4_aggregate_calc, "nsfms_Cmin_t4_aggregate_calc.csv")
