...
# LAST WORKED ON 3/9/2020 FOR CMIN T2 BY AP
...
cminT2 <- read_csv("raw-data/nsfms_Cmin_t2.csv")
cminT2$irga.integral <- as.numeric(cminT2$irga.integral)
...
cminT2[is.na(cminT2$std.value.1)==FALSE,] -> stds
...
right_join(stds,cminT2) %>% fill(meanStandard, .direction = "up") -> cminT2
...
cminT2 %>%
  ...
  ) %>% select(c(labID, plotID, soil.volume, CO2CperHour)) -> cminT2_calc 

write.csv(cminT2_calc, "cminT2_raw_calc.csv")

# 2. calculate co2 flux for standards
left_join(microcosmDesign, cminT2_calc, by = "labID") %>% 
...
left_join(microcosmDesign, cminT2_calc, by = "labID") %>% 
  ...
  select(plotID, microcosmNum, Rep, labID, moistTRT, CO2CpergLitter) -> cminT2_corrected_calc  
write.csv(cminT2_corrected_calc, "nsfms_Cmin_t2_calc.csv") 
...
aggregate(. ~ plotID + moistTRT,
          data =
            cminT2_corrected_calc %>%
            select(-c("microcosmNum", "Rep", "labID")),
          FUN = mean
) -> cminT2_aggregate_calc
write.csv(cminT2_aggregate_calc, "nsfms_Cmin_t2_aggregate_calc.csv")
